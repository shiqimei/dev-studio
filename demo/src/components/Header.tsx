...
export function Header() {
  ...
  return (
    <header className="px-5 py-2.5 border-b border-border flex items-center gap-3 shrink-0 min-w-0 overflow-hidden">
      <svg
      ...
      <h1 className="text-sm font-semibold text-text shrink-0">Claude Code ACP</h1>
      {sessionLabel && (
        <span className="flex-1 min-w-0 truncate text-xs text-dim">
          {sessionLabel}
        </span>
      )}
      <span id="status" className={`shrink-0 ${statusClass}`}>
        {state.connected
          ? "connected"
